...
def read_data(
    ...
    df = df.merge(
        op_unique_carrier_lookup, left_on=["OP_UNIQUE_CARRIER"], right_on=["Code"]
    ).rename(
        columns={
            "Description": "AIRLINE",
        }
...
def transform_data(X_train_raw, X_val_raw, X_test_raw):
    ...
    numerical = [
        ...
        "DAY_OF_WEEK_SIN",
        "DAY_OF_WEEK_COS",
        "CRS_DEP_TIME_MINUTES_SIN",
        "CRS_DEP_TIME_MINUTES_COS",
        "CRS_ARR_TIME_MINUTES_SIN",
...
def run_register_model(
    ...
    for run in runs:
        train_and_log_model(
            train, valid, test, y_val, y_test, dv, params=run.data.params
        )
...
def create_monitoring_report(reference_data, current_data, MLFLOW_S3_BUCKET):
    ...
    with open("reports/Monitoring_Report.html", "rb") as file:
        s3_client.upload_fileobj(
            file, MLFLOW_S3_BUCKET, "reports/Monitoring_Report.html"
        )
...
def main_flow():
    ...
    reference_data = get_monitoring_data(
        train, train_data_raw, client, MLFLOW_S3_BUCKET
    )
